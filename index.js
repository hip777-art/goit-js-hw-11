...
</li>`}function g(t){if(!o)return;if(!Array.isArray(t)||t.length===0){o.innerHTML="",s==null||s.refresh();return}const i=t.map(y).join("");o.innerHTML=i,s?s.refresh():s=new m(".gallery a",{captionsData:"title",captionDelay:250})}function p(){o&&(o.innerHTML="")}function h(){c&&c.classList.remove("is-hidden")}function L(){c&&c.classList.add("is-hidden")}searchForm.addEventListener("submit",t=>{t.preventDefault();const i=t.currentTarget.elements["search-text"].value.trim();if(i===""){u.warning({message:"Please enter a search query!"});return}p(),h(),getImagesByQuery(i).then(a=>{if(a.hits.length===0){u.error({message:"Sorry, there are no images matching your search query. Please try again!",position:"topRight"});return}g(a.hits)}).catch(a=>{console.error(a),u.error({message:"An error occurred. Please try again later."})}).finally(()=>{L(),t.target.reset()})});
